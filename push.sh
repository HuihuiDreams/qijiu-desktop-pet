...
# 安全检查：防止将未被忽略的扫描产物或内部工作区误提交（排除受版本控制的 .agents/skills/ 专属规范）
if git diff --cached --name-only --diff-filter=ACMR | grep -E '^(\.codex/|\.agents/|security-scans/)' | grep -v '^\.agents/skills/'; then
    echo -e "${RED}==========================================${NC}"
