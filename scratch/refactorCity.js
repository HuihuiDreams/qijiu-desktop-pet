...
// The new CitySettingWindow.js content
const cityCode = `const { BrowserWindow, screen, ipcMain } = require('electron');
...
fs.writeFileSync('src/main/windows/CitySettingWindow.js', cityCode);

// Regex removals from main.js
main = main.replace(/const CITY_SETTING_ALWAYS_ON_TOP_LEVEL = 'screen-saver';\n/, '');
main = main.replace(/const CITY_SETTING_TOP_PULSE_MS = 180;\n/, '');
main = main.replace(/let citySettingTopPulseTimer = null;\n/, '');

main = main.replace(/function pulseCitySettingWindowTop\(\) \{[\s\S]*?CITY_SETTING_TOP_PULSE_MS\);\s*\}/, '');
main = main.replace(/function raiseCitySettingWindow\(\) \{[\s\S]*?return windowManager\.citySettingWindow;\s*\}/, '');
main = main.replace(/function createCitySettingWindow\(\) \{[\s\S]*?return windowManager\.citySettingWindow;\s*\}/, '');
main = main.replace(/function openCitySettingWindow\(\) \{[\s\S]*?return raiseCitySettingWindow\(\);\s*\}/, '');
main = main.replace(/function closeCitySettingWindow\(\) \{[\s\S]*?\}\s*\}/, '');
main = main.replace(/ipcMain\.handle\('city-setting-close'[\s\S]*?\}\);\s*/, '');
...
main = main.replace(/openCitySettingWindow\(/g, "citySettingWindowModule.openCitySettingWindow(");
main = main.replace(/closeCitySettingWindow\(/g, "citySettingWindowModule.closeCitySettingWindow(");

fs.writeFileSync('main.js', main);
console.log('Removed City Setting logic from main.js');
