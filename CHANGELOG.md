...
- **状态窗口 `createStatusWindow` 引用缺失修复 (`Status Window Missing Module Reference`)**：修复了 `src/main/AppLifecycle.js` 中 `showStatusWindow` 直接调用裸函数 `createStatusWindow()` 导致的 `ReferenceError`，改为使用 `statusWindowModule.createStatusWindow()`，该函数在重构中已被迁移至 `src/main/windows/StatusWindow.js` 模块。
