...
- 修复 CP 屏保「被抓包」对话气泡显示时间过短（仅 800ms，一瞬即逝）的问题。解耦状态机冻结计时器（`CAUGHT_INDICATOR_DURATION_MS = 800ms`，控制宠物被抓包时的静止时长，不变）与气泡显示时长：新增独立常量 `CAUGHT_BUBBLE_DURATION_MS = 4000ms`，使气泡在宠物回到原位后仍可持续显示，与 `AmbientDialogueSystem`、`OfflineReturnSystem` 等其他功能的气泡时长保持一致（均为 4000–5000ms）。更新 `challengerStep3_1.test.js` 与 `screensaverOverlay.test.js` 中相关断言，804 tests pass 0 fail。
