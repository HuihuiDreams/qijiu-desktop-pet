...
### Fixed
- **Electron QA 启动环境兼容 (`Electron QA Launch Environment Compatibility`)**：Electron 冒烟测试与性能采样启动器会从子进程环境中移除 `ELECTRON_RUN_AS_NODE`，避免在 Codex 等将 Electron 模块作为 Node 运行的宿主环境中继承该变量后导致桌面应用启动即退出。
