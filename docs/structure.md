...
│  │  ├─ ScreensaverSystem.js           # CP 屏保渲染进程状态机与控制系统 (inactive | entering | performing | caught | runningBack)，复用 StageGeometry 的久坐提醒双宠居中布局并按其跨度扩展氛围层，连招结束后等待输入再显示 CSS 文本 ! 并回位；支持场景/DPI 组合缩放、皮肤 Overlay keys 校验跳过、runningBack 目标点 walkArea 夹紧与安全复位
