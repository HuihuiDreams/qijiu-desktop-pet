...
│  │  ├─ ScreensaverSystem.js           # CP 屏保渲染进程状态机与控制系统 (inactive | entering | performing | caught | runningBack)，复用 StageGeometry 的久坐提醒双宠居中布局并按其跨度扩展氛围层，连招结束后等待输入再展示至少 800ms 的 CSS 文本 ! 并回位；提示与场景共同应用 DPI 缩放，支持皮肤 Overlay keys 校验跳过、runningBack 目标点 walkArea 夹紧与安全复位
