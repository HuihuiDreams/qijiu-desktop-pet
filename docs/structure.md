...
- **天气与地理服务**：默认关闭以保护隐私；开启后基于 Open-Meteo 免 Key 接口，通过 `weatherSyncService.js` 发起，并在主进程清洗温度、WMO 天气代码、降水、雨量、阵雨、降雪、风速、风向和阵风字段。
...
- **局部天气渲染**：渲染进程收到天气特征（如 `rain`, `snow`, `clear`, `windy`, `thunderstorm`）和时段后，通过 `data-weather` 和 `data-time-phase` 保留状态语义；雨雪、大风和雷暴使用 `WeatherParticleLayer` 在桌宠附近创建有上限的局部粒子组，雷暴表现为雨粒子叠加低频局部闪电，大风表现为风痕粒子。动画交给 CSS `transform`，切换、隐藏或禁用时立即清理。天气效果不改变整屏亮度、对比度或饱和度。
...
- `protectedAssetLoader.js` 对已解密资源使用有大小上限的内存 LRU 缓存，并提供清理入口，避免同一皮肤帧在运行期重复读盘和解密。
- `protectedAssetProtocol.js` 注册 Electron `pet-asset://skin/...` 协议；`SkinManager`、`SpriteView`、`PetRenderer` 与番茄钟窗口都通过该协议加载运行时皮肤图片。
