...
- **局部天气渲染**：渲染进程收到天气特征（如 `rain`, `snow`, `clear`, `windy`, `thunderstorm`, `heat`）和时段后，通过 `data-weather` 和 `data-time-phase` 保留状态语义；雨雪、大风、雷暴和高温使用 `WeatherParticleLayer` 在桌宠附近创建有上限的局部粒子组，雷暴表现为雨粒子叠加低频局部闪电，大风表现为风痕粒子，炎热 (`heat`) 表现为自底向上升腾的清透白银阳炎折射流线与底座 (`top: 94%~100%`) 真实呼吸热辐射场，同时配合中英日三语 `weather_heat` 专属对话台词。动画交给 CSS `transform`，切换、隐藏或禁用时立即清理。天气效果不改变整屏亮度、对比度或饱和度。
