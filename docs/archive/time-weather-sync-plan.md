...
- MVP 天气表现只使用桌宠附近的局部 CSS/DOM 粒子、台词、现有 idle/rest/cultivate 状态；不改变整屏亮度、对比度或饱和度。
...
- 多显示器/DPI：特效层只跟随透明窗口、当前 display scale 和桌宠位置，不在每个 display 上重复创建独立全屏层。
...
| --------------- | ---------------- | ---------------------- | ------------------------------- |
| `clear`         | 晴、低云量       | 晒太阳台词、轻快 idle  | 不要求皮肤帧                    |
| `cloudy`        | 多云、雾         | 安静 idle、多云轻快台词 | 不要求皮肤帧                    |
| `overcast`      | 阴天             | 安静 idle、阴天压抑台词 | 不要求皮肤帧                    |
...
- 雨、雪优先放在透明窗口中的局部 effect layer，由 renderer 根据角色位置和 `scaleRatio` 处理；晴、阴、夜间优先通过台词和 idle/rest 倾向表达，不做全屏色彩滤镜。
...
**Description:** 在 renderer 中根据 `weatherKind` 和 `intensity` 添加轻量天气视觉表现。雨雪使用跟随桌宠位置的局部粒子效果；晴、阴和夜间保留 `data-weather` / `data-phase` 语义给台词、动作倾向和调试使用，但不改变整屏亮度、对比度或饱和度。

**MVP 基线（必须完成）：**
- [x] 根节点 `data-weather` / `data-phase` 属性随 payload 更新。
- [x] 雨/雪在桌宠附近显示局部粒子效果；晴/多云/深夜不做全屏视觉滤镜。
...
**可选增强（粒子特效，仅在基线稳定后评估）：**
- [x] 雨/雪粒子节点数量有配置上限（初始保守值，根据 DevTools 录制调整）。
...
- 🟡 雨/雪粒子默认上限已采用局部粒子组：雨 48、雪 40。后续根据 DevTools 录制和实际观感继续微调。
