...
- 连招采用现有通过素材校验的固定顺序 `hug → shareFood → kiss`；每轮结束后回到中心 idle 并从头开始。没有任何可用互动素材时，安全地停在中心 idle 等输入，不制造空循环。
...
**说明：** 将 `ScreensaverSystem` 的 `waiting_for_input` 终态改为循环边界：最后一个 Overlay 清理、idle 间隔结束后重置 `comboIndex` 并开始下一轮，不重新发起素材校验。普通输入将从 entering、任意循环阶段进入一次性的 caught 状态；重复 stop 忽略而非取消该反馈。修正感叹号位置使用 scene 的 display scale，并将 caught 展示时长提高至至少 600 ms；随后复用现有 `runningBack` 归位。静默 cancel 保持直接 reset。
