...
- 降低了因人为疏忽导致的版本混乱风险。
- 开发者需要适应使用脚本提交的新习惯。
- **后续更新 (2026-06-30)**: “推送前必须更新 `CHANGELOG.md`”仍是当前项目规则。最初的自动化落点是 Windows `push.ps1`；后续仓库也提供了 `push.sh`，并在 `docs/git-workflow.md` 中补充了 Windows、WSL、macOS 的跨平台 Git 配置与发布准备流程。因此，本 ADR 中关于 PowerShell 脚本的描述应理解为初始实现，不应排除等价的跨平台脚本或手动遵循同一检查流程。
