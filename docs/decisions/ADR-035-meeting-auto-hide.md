...
## Context
用户在 Teams 或 Zoom 开会时，桌宠可能遮挡共享屏幕、会议窗口或注意力焦点。MVP 需要在不请求额外权限、不读取会议内容、不依赖会议软件私有 API 的前提下，尽量判断用户是否正在使用已知会议应用开会，并在会议结束后自动恢复桌宠。
...
- Windows Zoom 是明确的例外：实测其会议中与离会后均为两个 `Zoom.exe`，UDP 数为 `0, 3`，无法通过 UDP 阈值判断。根据用户“打开 Zoom 即表示开会”的工作流，检测到任一 `Zoom.exe` 后直接返回结果、无需等待 `netstat`，并在首次采样立即判定为需要隐藏；该例外只使用进程名，不扩展到 macOS。
...
- Windows Zoom 按“打开即隐藏”的用户约定处理；macOS Zoom 及其他应用仍需实测校准 UDP 阈值。
